...
class CustomerOrderList extends StatefulWidget {
  ...
  const CustomerOrderList(
      {required this.customer, Key? key, required this.firestore})
      : super(key: key);
  ...
  Widget build(BuildContext context) {
    return StreamBuilder<List<Order>>(
        stream: OrderService(firestore: widget.firestore)
            .customerOrders(widget.customer.id),
              ...
              default:
                return Scaffold(
                    appBar: AppBar(
                      ...
                      title:
                          Text('${widget.customer.firstName}\'s Order History',
                    ...
                    body: (snapshot.data!.isNotEmpty)
                        ? StreamProvider<List<Order>>.value(
                            value: OrderService(firestore: widget.firestore)
                                .customerOrders(widget
                                    .customer.id), // as in wala koy mabuhat
                            initialData: const [],
